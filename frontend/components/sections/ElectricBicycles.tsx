...
import ElectricBicyclesClient from "./ElectricBicyclesClient";

export const revalidate = 180; // 3 minutes ISR

export default async function ElectricBicycles() {
    // Server-side fetch for promotions
    const [carsData, promotionsData] = await Promise.all([
        getCars({ type: 'bicycle', pageSize: 4 }), 
        getPromotions()
    ]);
    ...
    return (
        <ElectricBicyclesClient 
            initialBicycles={carsData}
